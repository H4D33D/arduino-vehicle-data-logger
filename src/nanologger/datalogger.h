//Responcible for Data Logging the data.
...
//------------------------------------------------
//Disabled, this is useless. Will keep if needed.
...
//Working Data Logger - Hadeeds Version.
//Original methods still avaliable.


void logData(uint16_t pid, int value) // New SPEED, RPM format
{
    ...
    Serial.write((uint8_t*)&ld, 12);
#else
    static int lastValue = -1; // Track the last value written
    if (lastValue != -1) {
    ...
    dataSize += 12;
#else
    static int count = 0; // Track the number of values written
    if (count % 2 == 1) {
        sdfile.println(); // Start a new line for every pair of values
    } else if (count != 0) {
        sdfile.print(", "); // Add a comma and space between values
    }
...
/*
void logData(uint16_t pid, int value) //SPEED,RPM,THROTTLE - OG Code that works.
{
#if LOG_FORMAT == FORMAT_BIN || STREAM_FORMAT == FORMAT_BIN
    LOG_DATA_COMM ld = {dataTime, pid, 1, 0, value};
    ld.checksum = getChecksum((char*)&ld, 12);
    ...
    dataSize += sdfile.print(value);
    sdfile.write(',');
    //sdfile.write('\n'); //I think this might work, because everything above does not apply to our csv format, HAVENT TESTED YET
#endif
#endif
}
*/
...
//----------------------------------------------------------------
/*
    void logData(uint16_t pid, float value) //Temps, Pressure, Voltage
    {
#if LOG_FORMAT == FORMAT_BIN || STREAM_FORMAT == FORMAT_BIN
        LOG_DATA_COMM ld = {dataTime, pid, 1, 0, value};
        ld.checksum = getChecksum((char*)&ld, 12);
...
//-------------------------------------------------
    ...
    void logData(uint16_t pid, float value1, float value2) //GPS COOR, ACC
    {
#if LOG_FORMAT == FORMAT_BIN || STREAM_FORMAT == FORMAT_BIN
...
//-------------------------------------------------
    ...
    void logData(uint16_t pid, uint32_t value1, uint32_t value2) //GPS TIME, FILE INFO
    {
#if LOG_FORMAT == FORMAT_BIN || STREAM_FORMAT == FORMAT_BIN
        LOG_DATA_COMM ld = {dataTime, pid, 2, 0, {value1, value2}};
        ld.checksum = getChecksum((char*)&ld, 16);
